...
th{background:#112447}.actions{display:grid;grid-template-columns:repeat(2,minmax(92px,1fr));gap:6px;min-width:200px}.actions button{width:100%;white-space:nowrap}.muted{color:#94a3b8;font-size:12px}.key,.device{direction:ltr;font-family:Consolas,monospace}.active{color:#34d399}.expired,.suspended{color:#f87171}.device_mismatch{color:#f59e0b}
...
<table><thead><tr><th>إجراءات</th><th>العميل</th><th>المفتاح</th><th>الانتهاء</th><th>الحالة</th><th>الجهاز / Device ID</th><th>آخر ظهور</th></tr></thead><tbody id="rows"></tbody></table></div>
<script>
...
async function loadLicenses(){try{const data=await api('/api/admin/licenses');rows.innerHTML=data.licenses.map(l=>'<tr><td><div class="actions"><button class="ok" onclick="renew(\\''+l.id+'\\')">تجديد سنة</button><button class="ok" onclick="renewCustom(\\''+l.id+'\\')">تجديد مخصص</button><button class="warn" onclick="releaseDevice(\\''+l.id+'\\')">فك الجهاز</button><button class="danger" onclick="suspend(\\''+l.id+'\\')">إيقاف</button><button onclick="activate(\\''+l.id+'\\')">تشغيل</button><button class="danger" onclick="deleteLicense(\\''+l.id+'\\',\\''+esc(l.customerName)+'\\')">حذف نهائي</button></div></td><td>'+esc(l.customerName)+'<div class="muted">'+esc(l.phone)+'</div></td><td class="key">'+esc(l.licenseKey)+'</td><td>'+esc(l.expiresAt.slice(0,16).replace('T',' '))+'</td><td class="'+esc(l.status)+'">'+esc(l.status)+'</td><td>'+esc(l.deviceName||'غير مفعل')+'<div class="device muted">'+esc(l.deviceFingerprint||'لم يربط بعد')+'</div></td><td>'+esc(l.lastSeenAt?new Date(l.lastSeenAt).toLocaleString('ar-EG'):'-')+'</td></tr>').join('');setMessage(data.licenses.length?'تم تحميل التراخيص':'لا توجد تراخيص بعد');}catch(err){setMessage('خطأ: '+err.message,'err');}}
...
async function deleteLicense(id,name){if(confirm('تحذير: سيتم حذف العميل '+name+' ومفتاحه نهائيا من السيرفر. لا يمكن التراجع. متابعة؟')){try{await api('/api/admin/licenses/'+id+'/delete',{method:'POST'});await loadLicenses();setMessage('تم حذف العميل والمفتاح نهائيا','ok');}catch(err){setMessage('فشل الحذف: '+err.message,'err');}}}
loadLicenses();
</script></body></html>`);
});
...
app.post('/api/admin/licenses/:id/delete', requireAdmin, async (req, res) => {
  const db = await readDb();
  const before = db.licenses.length;
  db.licenses = db.licenses.filter((item) => item.id !== req.params.id);
  if (db.licenses.length === before) return res.status(404).json({ message: 'Not found' });
  await writeDb(db);
  res.json({ ok: true });
});
